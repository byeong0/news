...
"""
뉴스 생성 스크립트 (Gemini)
Gemini API (google-genai SDK)를 사용하여 AI 관련 뉴스를 생성하고 Jekyll 포스트로 저장합니다.
"""
...
def generate_news():
    ...
    try:
        ...
        if response.candidates and response.candidates[0].grounding_metadata:
             ...
             # Search Entry Point 확인
             if response.candidates[0].grounding_metadata.search_entry_point:
                 print(f"Search Entry Point: {response.candidates[0].grounding_metadata.search_entry_point.rendered_content}")
             # Grounding Chunks 확인
             if response.candidates[0].grounding_metadata.grounding_chunks:
                 for i, chunk in enumerate(response.candidates[0].grounding_metadata.grounding_chunks):
                     if chunk.web:
                         print(f"Chunk {i}: {chunk.web.title} - {chunk.web.uri}")
        else:
             print("\n[No Grounding Metadata Found - Search might not have been used]")
        ...
        # Jekyll 포스트 기본 디렉토리
        posts_dir = project_root / "_posts" / "gemini" / "ai_news"
