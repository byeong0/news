#!/usr/bin/env python3
"""
뉴스 생성 스크립트 (Perplexity)
Perplexity API를 사용하여 AI 관련 뉴스를 생성하고 Jekyll 포스트로 저장합니다.
"""
...
def generate_news():
    ...
    if not api_key:
        print("Error: PERPLEXITY_API_KEY 환경 변수가 설정되지 않았습니다.")
        sys.exit(1)
    
    # OpenAI 클라이언트 초기화 (Perplexity Base URL 사용)
    client = OpenAI(
        api_key=api_key,
    ...
    try:
        ...
        messages = [
            {
                "role": "system",
                "content": "You are a news generator. Output ONLY the news articles in the specified markdown format. Do not provide introductions, conclusions, or conversational text. If no news is found, just say 'No news found'."
            },
            {
                "role": "user",
                "content": prompt
        ...
        if not blog_content or not blog_content.strip():
            print("해당 날짜에 발행된 주요 AI 뉴스가 없습니다. (빈 응답)")
            return None
        ...
        # Jekyll 포스트 기본 디렉토리
        posts_dir = project_root / "_posts" / "perplexity" / "ai_news"
